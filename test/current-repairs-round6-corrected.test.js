...
} = require("../src/current-repairs-round6-corrected");
...
surfaceTest("Surface PR #16 closes the original own-key rewrite and preserves alternate inherited-looking own data", () => {
...
coreTest("MorphTile core PR #15 preserves authored own recipe keys and blocks absent inherited host names from becoming authority", () => {
...
interfaceTest("Interface PR #13 keeps canonical meter semantics and current Assembly preserves path-bound proof closure without inventing nested kit context", () => {
  ...
  const result = verifyInterfaceCurrentAssemblyCorrected(Interface, Assembly, materializeKit, MorphTile, integrationSources, {
  ...
  assert.equal(result.receipt.nested.portable_candidate_equal, true);
  assert.equal(result.receipt.nested.target_proofs_exact, true);
  assert.equal(result.receipt.nested.closure_hashes_differ_due_to_target_local_dependencies, true);
  assert.equal(result.receipt.nested.kit_status, "HOLD");
  assert.equal(result.receipt.nested.kit_hold, "HOLD_KIT_DEPENDENCY_UNSATISFIED");
  assert.equal(result.receipt.nested.proof_status, "UNSATISFIED");
  assert.equal(result.receipt.local_positive.assembly_status, "CANDIDATE");
  assert.equal(result.receipt.local_positive.kit_status, "CANDIDATE");
  assert.equal(result.receipt.local_positive.import_status, "READY");
  assert.equal(result.receipt.visual_quality, "NOT_TESTED");
});

assemblyTest("Assembly PR #21 preserves Form 0.11 rotation plus definition closure through hash tamper rejection, fresh import, and runtime meaning", () => {
  ...
  const MorphTile = require(path.resolve(coreMainPath));
  const result = verifyAssemblyFormRotationCorrected(Form, Assembly, materializeKit, MorphTile, {
