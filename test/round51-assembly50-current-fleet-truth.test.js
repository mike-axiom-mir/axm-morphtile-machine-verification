...
const HISTORICAL_FORM = "dd5764efdcd75bb826dd908a4d2245ad04d0a57a";
const HISTORICAL_INTERFACE = "ddafc9e30144d12605c6e4198cd8d6c5764e6212";
const HISTORICAL_ASSEMBLY49 = "fea9f0b7ba5a8c4bf6710806e636f53f6a6a72cc";
const OPEN_INTERFACE45 = "948e6aeca7426a2f3b80cf5904640c943713ea17";
...
test("Assembly 50 exact head is only a current-fleet/status truth refresh", { skip: !enabled }, () => {
  ...
  assert.equal(
    git(process.env.R51_ASSEMBLY50_ROOT, ["diff", "--name-only", ASSEMBLY_BASE, ASSEMBLY50, "--", "src", "machine.json", "package.json", "scripts", "test", ".github"]),
    "",
    "fleet receipt refresh must not hide runtime, executable evidence, package or workflow movement"
  );
});

test("Assembly 50 manifest matches independently checked-out integrated heads and executable fleet grammar", { skip: !enabled }, () => {
  ...
  assert.deepEqual([...pins.LANES], ["form", "surface", "capability", "interface", "core"], "manifest keys diverged from executable current-fleet grammar");
  ...
  assert.equal(
    ...
    "pin emission diverged from the exact validated current fleet"
  );
});

test("Assembly 50 status keeps current integrated truth separate from historical receipts and open work", { skip: !enabled }, () => {
  ...
  assert.ok(status.includes(marker), "status lost round-41 current candidate section");
  ...
  assert.ok(status.includes(HISTORICAL_ASSEMBLY49), "status no longer anchors the predecessor receipt to its exact historical head");
  assert.ok(!current.includes(HISTORICAL_ASSEMBLY49), "historical Assembly #49 head is presented as current fleet truth");
  assert.ok(!current.includes(HISTORICAL_FORM), "historical Form identity is presented as current");
  assert.ok(!current.includes(HISTORICAL_INTERFACE), "historical Interface identity is presented as current");
  assert.ok(!current.includes(OPEN_INTERFACE45), "open Interface #45 was prematurely adopted into integrated fleet truth");
  assert.match(status, /fresh independent Verification before Creation Director integration/i);
});
