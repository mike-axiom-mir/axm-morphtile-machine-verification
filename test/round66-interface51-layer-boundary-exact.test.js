...
function request(placement) {
  return {
    ...
    request_id: 'verification-round66-layer-boundary',
    ...
    provenance: { caller: 'verification-round66' }
  };
}

test('round66 exact Interface51 presentation-layer substrate boundary', { timeout: 90000 }, () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'axm-round66-'));
  try {
