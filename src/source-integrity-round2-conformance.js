...
function verifySurfaceRepairAndIntentBoundary(surface, options = {}) {
  ...
  // Replay the exact class that failed on the previous Surface revision: an
  // accessor inside caller-owned paint. The repaired revision must reject it
  ...
  const previousAttackPass = nestedOut.status === "HOLD"
    ...
    && nestedDescriptorAfter.enumerable === nestedDescriptorBefore.enumerable
    && nestedDescriptorAfter.configurable === nestedDescriptorBefore.configurable;
  if (!previousAttackPass) {
    errors.push(failure(
      "SURFACE_REPAIRED_PAINT_ACCESSOR_REPLAY_FAILED",
      "The repaired Surface revision did not close the previously observed paint-field accessor execution boundary.",
      { status: nestedOut.status, hold: nestedHold, accessor_calls: nestedAccessorCalls }
    ));
  }
  checked.push("previous-paint-accessor-attack-replayed-on-repaired-head");

  // Move the same source-integrity attack one ownership boundary outward. A
  // caller may place an accessor on intent.paint itself. Surface must decide
  // whether authored intent is plain portable data before reading that field.
  ...
  const intentSourceSafe = !intentThrew
    ...
    && intentHold === "HOLD_SURFACE_PAINT_NONPORTABLE_VALUE"
    && intentOut.candidate === null
    && intentAccessorCalls === 0
    && intentDescriptorAfter
    && intentDescriptorAfter.get === intentDescriptorBefore.get
    && intentDescriptorAfter.enumerable === intentDescriptorBefore.enumerable
    && intentDescriptorAfter.configurable === intentDescriptorBefore.configurable;
  if (!intentSourceSafe) {
    errors.push(failure(
      "SURFACE_INTENT_FIELD_ACCESSOR_EXECUTED_BEFORE_HOLD",
      "Surface did not establish a source-safe descriptor boundary before reading caller-owned intent.paint.",
      {
  ...
  if (!portableControlPass) {
    errors.push(failure(
      "SURFACE_ROUND2_PORTABLE_CONTROL_DRIFT",
      "The source-integrity boundary rejected or rewrote an ordinary portable paint control.",
      { status: portableOut.status }
    ));
  }
...
function verifyFormPreSerializationBoundary(form, options = {}) {
  const formCommit = options.formCommit || null;
  const errors = [];
  const checked = [];

  if (!formCommit) {
    return { status: "FAIL", checked, errors: [failure("FORM_REVISION_PIN_MISSING", "Exact Form revision is required.")], receipt: null };
  }
  if (!form || typeof form.run !== "function") {
    return { status: "FAIL", checked, errors: [failure("FORM_RUN_CONTRACT_MISSING", "Form verification requires run().")], receipt: null };
  }
  ...
  if (accessorOut.status !== "HOLD" || accessorHold !== "HOLD_FORM_INPUT_NONPORTABLE_VALUE" || accessorOut.candidate !== null || intentAccessorCalls !== 0
      || !accessorDescriptorAfter || accessorDescriptorAfter.get !== accessorDescriptorBefore.get) {
    errors.push(failure(
      "FORM_ROOT_ACCESSOR_SOURCE_BOUNDARY_FAILED",
      "Form did not reject a root request accessor without executing caller code.",
      { status: accessorOut.status, hold: accessorHold, accessor_calls: intentAccessorCalls }
    ));
  }
  ...
  if (negativeZeroOut.status !== "HOLD" || negativeZeroHold !== "HOLD_FORM_INPUT_NONPORTABLE_VALUE" || negativeZeroPath !== "request.intent.recipe[0].pos[0]" || negativeZeroOut.candidate !== null) {
    errors.push(failure(
      "FORM_NEGATIVE_ZERO_PORTABILITY_BOUNDARY_FAILED",
      "Form did not fail closed on authored -0 before JSON transport could rewrite it to 0.",
      { status: negativeZeroOut.status, hold: negativeZeroHold, path: negativeZeroPath }
    ));
  }
  ...
  if (!portablePass) {
    errors.push(failure(
      "FORM_ROUND2_PORTABLE_CONTROL_DRIFT",
      "Form source-integrity preflight rejected, mutated, or rewrote an ordinary portable caller recipe.",
      { status: portableOut.status }
    ));
  }
...
function verifyAssemblyPreSerializationBoundary(assembly, kitModule, MorphTile, options = {}) {
  const assemblyCommit = options.assemblyCommit || null;
  const coreCommit = options.coreCommit || null;
  const errors = [];
  const checked = [];

  if (!assemblyCommit || !coreCommit) {
    return { status: "FAIL", checked, errors: [failure("ASSEMBLY_CORE_REVISION_PIN_MISSING", "Exact Assembly and MorphTile revisions are required.")], receipt: null };
  }
  if (!assembly || typeof assembly.run !== "function" || !kitModule || typeof kitModule.materializeKit !== "function" || !MorphTile) {
    return { status: "FAIL", checked, errors: [failure("ASSEMBLY_CONTRACT_MISSING", "Assembly run(), materializeKit(), and MorphTile runtime are required.")], receipt: null };
  }
  ...
  Object.defineProperty(accessorRequest, "inputs", {
    ...
    get() {
      ...
      return [{
        status: "CANDIDATE",
        candidate: { schema: "morphtile.tile-spec/v0.4", id: "mt_portable", form_hints: [], facets: {} }
      }];
    }
  });
  ...
  if (!requestBoundaryPass) {
    errors.push(failure(
      "ASSEMBLY_REQUEST_ACCESSOR_SOURCE_BOUNDARY_FAILED",
      "Assembly did not reject a root inputs accessor without executing caller code.",
      { status: accessorOut.status, hold: accessorHold, accessor_calls: requestAccessorCalls }
    ));
  }
  ...
  if (!portableRequestPass) {
    errors.push(failure(
      "ASSEMBLY_ROUND2_PORTABLE_REQUEST_DRIFT",
      "Assembly source-integrity preflight rejected or rewrote a portable closure control.",
      { status: portableOut.status }
    ));
  }
  checked.push("portable-assembly-request-remains-candidate");

  // A nested Interface address must still fail closed without explicit parent
  // context; source-integrity preflight must not accidentally normalize this
  // semantic HOLD away.
  const unboundNestedOut = assembly.run(baseEnvelope(
    ...
    { id: "mt_inner", name: "Nested target" },
    {
      inputs: [
        {
          status: "CANDIDATE",
          candidate: { schema: "morphtile.tile-spec/v0.4", id: "mt_inner", form_hints: ["ui_panel"], facets: {} }
        },
        {
          status: "CANDIDATE",
          candidate: {
            schema: "morphtile.view-operation/v0.5",
            operation: { op: "view.set", id: "mt_shell/mt_inner", view: { title: "Nested", body: [] } }
          }
        }
      ]
    }
  ...
  if (!unboundNestedPass) {
    errors.push(failure(
      "ASSEMBLY_SOURCE_PREFLIGHT_WIDENED_PARENT_CONTEXT",
      "Assembly no longer held a nested Interface target whose parent path was not explicitly bound.",
      { status: unboundNestedOut.status, holds: unboundNestedOut.holds }
    ));
  }
  checked.push("nested-parent-context-remains-explicit-and-uninvented");

  // Kit boundary: reject an accessor on a supplied result before sourceTrace or
  // runtime translation can read it.
  const forgedResult = assemblyPortableRequest("verification-assembly-kit-forged-source").inputs[0];
  ...
  if (!kitResultBoundaryPass) {
    errors.push(failure(
      "ASSEMBLY_KIT_RESULT_ACCESSOR_SOURCE_BOUNDARY_FAILED",
      "Kit materialization did not reject an accessor-backed supplied Assembly result without executing caller code.",
      { status: forgedOut.status, hold: forgedHold, path: forgedPath, accessor_calls: kitResultAccessorCalls }
    ));
  }
  checked.push("kit-result-accessor-rejected-before-source-trace");

  // Generate a real portable Assembly result, then attack kit options separately.
  ...
  const optionsOut = kitModule.materializeKit(kitSource, MorphTile, options);
  ...
  if (!kitOptionsBoundaryPass) {
    errors.push(failure(
      "ASSEMBLY_KIT_OPTIONS_ACCESSOR_SOURCE_BOUNDARY_FAILED",
      "Kit materialization did not reject accessor-backed options without executing caller code.",
      { status: optionsOut.status, hold: optionsHold, path: optionsPath, accessor_calls: optionsAccessorCalls }
    ));
  }
  ...
  const kitControlPass = kitControlOut.status === "CANDIDATE"
    ...
    && kitControlOut.holds && kitControlOut.holds.length === 0;
  if (!kitControlPass) {
    errors.push(failure(
      "ASSEMBLY_KIT_PORTABLE_CONTROL_FAILED",
      "A normal portable Assembly result no longer materialized to a fresh-import-verified MorphTile kit.",
      { status: kitControlOut.status, holds: kitControlOut.holds }
    ));
  }
  ...
  return {
    ...
    receipt: {
      ...
      request_accessor: {
        ...
        pass: requestBoundaryPass
      },
      portable_request: {
        status: portableOut.status,
        preserved: portableRequestPass
      },
      nested_parent_context: {
        status: unboundNestedOut.status,
        hold_preserved: unboundNestedPass
      },
      kit_result_accessor: {
        ...
        pass: kitResultBoundaryPass
      },
      kit_options_accessor: {
        status: optionsOut.status,
        hold: optionsHold,
        path: optionsPath,
        accessor_calls: optionsAccessorCalls,
        pass: kitOptionsBoundaryPass
      },
      portable_kit_control: {
        status: kitControlOut.status,
        format: kitControlOut.kit && kitControlOut.kit.format || null,
        pass: kitControlPass
      }
    }
  };
